...
NOUN_PHRASE: ARTICLE NOUN { $$ = strcat(strdup($1), strdup($2)); }
           | ARTICLE NOUN PREP_PHRASE { $$ = strcat(strdup(strcat($1, $2)), $3); }
;

// Definición de una frase verbal
VERB_PHRASE: VERB { $$ = strdup($1); }
           | VERB NOUN_PHRASE { $$ = strcat(strdup($1), strdup($2)); }
;

// Definición de una frase preposicional
PREP_PHRASE: PREP NOUN_PHRASE { $$ = strcat(strdup($1), strdup($2)); }
;
